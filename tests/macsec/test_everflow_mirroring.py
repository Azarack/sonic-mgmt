...
SONiC - Macsec - Verify Everflow Encryption Mirroring
...
from tests.everflow import everflow_test_utilities as everflow_utils

from tests.everflow.everflow_test_utilities import BaseEverflowTest
from tests.everflow.everflow_test_utilities import TEMPLATE_DIR, EVERFLOW_RULE_CREATE_TEMPLATE, \
...
def setup_mirror_session_dest_ip_route(tbinfo, setup_info, apply_mirror_session):       # noqa F811
    ...
    logger.info(f"dest port list: {dest_port_ptf_id_list}| setup port: {setup_info[UP_STREAM]['dest_port_ptf_id']}")
...
# @pytest.fixture(scope='module')
def apply_acl_rule(setup_info, tbinfo, ip_ver):     # noqa F811
    """
...
def test_everflow_per_interface(ptfadapter, setup_info, tbinfo, setup_mirror_session_dest_ip_route,                
    ...
    logger.info(f"Ports to use: down {down_port} up {up_port}")
    ...
    for port, ptf_idx in list(everflow_config['candidate_ports'].items()):
        logger.info("Verifying packet ingress from {} with index {} is mirrored".format(port, ptf_idx))
    ...
    # Configure Macsec and verify mirroring
