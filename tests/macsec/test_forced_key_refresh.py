...
def setup(tbinfo, nbrhosts, duthosts, enum_frontend_dut_hostname, request):
    ...
    logger.info("lldp table: {}".format(lldp_table))
    neigh1 = lldp_table[1]
    ...
    logger.info("Neighbor BGP Config: {}".format(nbrhosts[neigh1]["host"].shell("show run bgp")['stdout']))
...
def test_forced_key_refresh(setup):
    ...
    cmd = 'vtysh -n {} -c "test macsec mka rekey interface {}"'.format(ns, setup['dut_int'])
